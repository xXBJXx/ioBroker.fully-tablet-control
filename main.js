...
class FullyTabletControl extends utils.Adapter {
            ...
            for (const obj in objects) {
                switch (obj) {
                    ...
                    case 'screenOrientation': {
                        ...
                        this.setState(`device.${deviceID}.device_info.screenOrientation`, {
                    ...
                    case 'deviceManufacturer': {
                        ...
                        this.setState(`device.${deviceID}.device_info.deviceManufacturer`, {
                    ...
                    case 'keyguardLocked': {
                        ...
                        this.setState(`device.${deviceID}.device_info.keyguardLocked`, {
                            val: keyguardLocked,
                            ack: true,
                        });
                    ...
                    case 'isDeviceAdmin': {
                        ...
                        this.setState(`device.${deviceID}.device_info.isDeviceAdmin`, {
                            val: isDeviceAdmin,
                            ack: true,
                        });
                    ...
                    case 'isDeviceOwner': {
                        ...
                        this.setState(`device.${deviceID}.device_info.isDeviceOwner`, {
                            val: isDeviceOwner,
                            ack: true,
                        });
                    ...
                    case 'displayWidthPixels': {
                        ...
                        this.setState(`device.${deviceID}.device_info.displayWidthPixels`, {
                            val: displayWidthPixels,
                            ack: true,
                        });
                    ...
                    case 'androidVersion': {
                        ...
                        this.setState(`device.${deviceID}.device_info.androidVersion`, {
                            val: androidVersion,
                            ack: true,
                        });
                    ...
                    case 'batteryLevel': {
                        ...
                        const plugged = objects['plugged'] ? objects['plugged'] : objects['isPlugged'];
                    ...
                    case 'ssid': {
                        ...
                        if (ssid === '<unknown ssid>') {
                            this.setState(`device.${deviceID}.device_info.ssid`, {
                    ...
                    case 'foregroundApp': {
                        ...
                        this.setState(`device.${deviceID}.device_info.foregroundApp`, {
                            val: foreground[index],
                            ack: true,
                        });
                        ...
                        if (
                            (await foreground[index]) !== 'de.ozerov.fully' &&
                            (await foregroundStart[index]) === false
                        ) {
                    ...
                    case 'internalStorageFreeSpace': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.internalStorageFreeSpace`, {
                            val: await this.formatBytes(internalStorageFreeSpace),
                            ack: true,
                        });
                    ...
                    case 'appTotalMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.appTotalMemory`, {
                            val: await this.formatBytes(appTotalMemory),
                            ack: true,
                        });
                    ...
                    case 'ramFreeMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.ramFreeMemory`, {
                            val: await this.formatBytes(ramFreeMemory),
                            ack: true,
                        });
                    ...
                    case 'appFreeMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.appFreeMemory`, {
                            val: await this.formatBytes(appFreeMemory),
                            ack: true,
                        });
                    ...
                    case 'internalStorageTotalSpace': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.internalStorageTotalSpace`, {
                            val: await this.formatBytes(internalStorageTotalSpace),
                            ack: true,
                        });
                        this.log.debug(
                            `internalStorageTotalSpace state for ${deviceID} : ${internalStorageTotalSpace}`,
                        );
                    ...
                    case 'ramUsedMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.ramUsedMemory`, {
                            val: await this.formatBytes(ramUsedMemory),
                            ack: true,
                        });
                    ...
                    case 'appUsedMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.appUsedMemory`, {
                            val: await this.formatBytes(appUsedMemory),
                            ack: true,
                        });
                    ...
                    case 'ramTotalMemory': {
                        ...
                        this.setState(`device.${deviceID}.device_info.memory.ramTotalMemory`, {
                            val: await this.formatBytes(ramTotalMemory),
                            ack: true,
                        });
                    ...
                    case 'batteryTemperature': {
                        ...
                        this.setState(`device.${deviceID}.device_info.batteryTemperature`, {
                        ...
                        if (motionDetectorStatusObj !== undefined || motionDetectorStatusObj !== null) {
                            switch (motionDetectorStatusObj) {
                                case 0:
                                    ...
                                    this.log.debug(
                                        `motionDetectorStatus state for ${deviceID} : ${motionDetectorStatusObj} = ${motionDetectorStatus[deviceID]}`,
                                    );
                                    break;
                                ...
                                case 1:
                                    // console.log(`motionDetectorStatus state for ${deviceID} : ${motionDetectorStatusObj} = ${motionDetectorStatus[deviceID]}`)
                                    this.log.debug(
                                        `motionDetectorStatus state for ${deviceID} : ${motionDetectorStatusObj} `,
                                    );
                                    break;
            ...
            switch (cmd) {
                case 'reloadAll': {
                    ...
                    await axios
                        ...
                        .then(async (result) => {
                            this.log.debug(
                                `${tabletName[index]} send status for reloadAll = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for reloadAll = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for reloadAll could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for reloadAll could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                    break;
                }
                ...
                case 'setStringSetting': {
                    ...
                    if (txtKey.length > 1) {
                        ...
                        await axios
                            .get(setStringSetting)
                            .then(async (result) => {
                                this.setState(id, { val: '', ack: true });
                                this.log.debug(
                                    `${tabletName[index]} send status for setStringSetting = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for setStringSetting = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for setStringSetting could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for setStringSetting could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                    ...
                    if (volume >= 0 && volume <= 100) {
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                ...
                                this.log.debug(
                                    `${tabletName[index]} send status for mediaVolume = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for mediaVolume = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for mediaVolume could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for mediaVolumen could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                    ...
                    if (txtSp.length > 1) {
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                this.setState(id, { val: '', ack: true });
                                this.log.debug(
                                    `${tabletName[index]} send status for textToSpeech = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for textToSpeech = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for textToSpeech could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for textToSpeech could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                ...
                case 'startURL': {
                    ...
                    const result = startURL.match('http');
                    if (!result) {
                    ...
                    if (startURL.length > 5) {
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                ...
                                this.log.debug(
                                    `${tabletName[index]} send status for startURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for startURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for startURL could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for startURL could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                    ...
                    if (strUrl.length > 5) {
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                this.setState(id, { val: '', ack: true });
                                this.log.debug(
                                    `${tabletName[index]} send status for loadURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for loadURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for loadURL could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for loadURL could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                ...
                case 'startApplication': {
                    ...
                    if (strApp.length > 2) {
                        ...
                        await axios
                            .get(startApplicationURL)
                            .then(async (result) => {
                                this.setState(id, { val: '', ack: true });
                                this.log.debug(
                                    `${tabletName[index]} send status for startApplication = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for startApplication = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for startApplication could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for startApplication could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
                ...
                case 'motionDetection': {
                    ...
                    await axios
                        .get(motionDetectionURL)
                        .then(async (result) => {
                            this.setState(id, { val: motionD, ack: true });
                            this.log.debug(
                                `${tabletName[index]} send status for motionDetection = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for motionDetection = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for motionDetection could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for motionDetection could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'camshot': {
                    try {
                        ...
                        if (series_shot > series_shot_safe) {
                            this.log.debug(
                                `The number of series of images to be captured is greater than the number of images to be saved.`,
                            );
                            series_shot = series_shot_safe;
                            this.log.debug(
                                `The number of series of image recording has been adjusted to the number of images to be saved.`,
                            );
                        }
                        ...
                        if (auto_motionDetection) {
                            ...
                            let motionDetection = await this.getStateAsync(
                                `device.${tabletName[index]}.commands.motionDetection`,
                            );
                            // @ts-ignore
                            ...
                            if (motionDetectorStatus[deviceID] && !motionDetection) {
                                ...
                                this.setState(`device.${tabletName[index]}.commands.motionDetection`, {
                                    val: true,
                                    ack: true,
                                });
                                ...
                                const motionDetectionURL = `http://${ip[index]}:${
                                    port[index]
                                }/?cmd=setBooleanSetting&key=motionDetection&value=${true}&password=${password[index]}`;
                                await axios
                                    .get(motionDetectionURL)
                                    .then(async (result) => {
                                        this.log.debug(
                                            `${tabletName[index]} send status for camshot = status Code: ${result.status} => status Message: ${result.statusText}`,
                                        );
                                        // console.log(`${tabletName[index]} send status for camshot = status Code: ${result.status} => status Message: ${result.statusText}`);
                                    })
                                    .catch(async (error) => {
                                        this.log.error(
                                            `${tabletName[index]} send status for camshot could not be sent => ${error.message}, stack: ${error.stack}`,
                                        );
                            ...
                            if (motionDetection) {
                                ...
                                this.log.warn(
                                    `Attention the motion detection is not activated it is not possible to take camshot if motion detection is deactivated !!`,
                                );
                            }
                        }
                    } catch (error) {
                        this.log.error(
                            `camshot function for ${tabletName[index]} has a problem: ${error.message}, stack: ${error.stack}`,
                        );
                    }
                    break;
                }
                ...
                case 'kioskPin_confirm': {
                    ...
                    const pin = (
                        await this.getStateAsync(
                            `${this.namespace}.device.${tabletName[index]}.commands.kiosk.kioskPin`,
                        )
                    ).val;
                        ...
                        if (pinConfirm === pin) {
                            ...
                            await axios
                                ...
                                .then(async (result) => {
                                    ...
                                    this.log.debug(
                                        `${tabletName[index]} send status for kioskPin = status Code: ${result.status} => status Message: ${result.statusText}`,
                                    );
                                    console.log(
                                        `${tabletName[index]} send status for kioskPin = status Code: ${result.status} => status Message: ${result.statusText}`,
                                    );
                                ...
                                .catch(async (error) => {
                                    this.log.error(
                                        `${tabletName[index]} send status for kioskPin could not be sent => ${error.message}, stack: ${error.stack}`,
                                    );
                                    console.log(
                                        `${tabletName[index]} send status for kioskPin could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                    );
                                });
                        } else {
                    ...
                    // @ts-ignore
                        ...
                        this.setState(`${this.namespace}.device.${tabletName[index]}.commands.kiosk.kioskPin`, {
                            val: 'Attention kiosk pin may only contain numbers (1234567890) do not contain any special characters or letters',
                            ack: true,
                        });
                        this.log.warn(
                            `Attention kiosk pin may only contain numbers (1234567890) do not contain any special characters or letters`,
                        );
                    } else if (isNaN(pinConfirm)) {
                        this.setState(id, {
                            val: 'Attention kiosk pin Confirm may only contain numbers (1234567890) do not contain any special characters or letters',
                            ack: true,
                        });
                        this.log.warn(
                            `Attention kiosk pin Confirm may only contain numbers (1234567890) do not contain any special characters or letters`,
                        );
                    }
                ...
                case 'disableStatusBar': {
                    ...
                    await axios
                        .get(disableStatusBarURL)
                        .then(async (result) => {
                            this.setState(id, { val: disableStatusBar, ack: true });
                            this.log.debug(
                                `${tabletName[index]} send status for disableStatusBar = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableStatusBar = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for disableStatusBar could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableStatusBar could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'disableHomeButton': {
                    ...
                    await axios
                        .get(disableHomeButtonURL)
                        .then(async (result) => {
                            this.setState(id, { val: disableHomeButton, ack: true });
                            this.log.debug(
                                `${tabletName[index]} send status for disableHomeButton = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableHomeButton = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for disableHomeButton could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableHomeButton could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'disablePowerButton': {
                    ...
                    await axios
                        .get(disablePowerButtonURL)
                        .then(async (result) => {
                            this.setState(id, { val: disablePowerButton, ack: true });
                            this.log.debug(
                                `${tabletName[index]} send status for disablePowerButton = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disablePowerButton = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for disablePowerButton could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disablePowerButton could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'disableVolumeButtons': {
                    ...
                    await axios
                        .get(disableVolumeButtonsURL)
                        .then(async (result) => {
                            this.setState(id, { val: disableVolumeButtons, ack: true });
                            this.log.debug(
                                `${tabletName[index]} send status for disableVolumeButtons = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableVolumeButtons = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for disableVolumeButtons could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for disableVolumeButtons could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'kioskMode': {
                    ...
                    await axios
                        ...
                        .then(async (result) => {
                            ...
                            this.log.debug(
                                `${tabletName[index]} send status for setKioskMode = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for setKioskMode = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for setKioskMode could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for setKioskMode could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                case 'lockKiosk': {
                    ...
                    await axios
                        ...
                        .then(async (result) => {
                            ...
                            this.log.debug(
                                `${tabletName[index]} send status for lockKiosk = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for lockKiosk = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            this.log.error(
                                `${tabletName[index]} send status for lockKiosk could not be sent => ${error.message}, stack: ${error.stack}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for lockKiosk could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                            );
                        });
                ...
                default:
                    if (state.val === true) {
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                this.log.debug(
                                    `${tabletName[index]} send status for ${cmd} = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for ${cmd} = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for ${cmd} could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for ${cmd} could not be sent [ command: ${cmd} val: ${state.val} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                    }
                    break;
    ...
    async image_capture() {
        try {
            ...
            const base64 = await axios
                .get(
                    `http://${ip[shareObj.index]}:${port[shareObj.index]}/?cmd=getCamshot&password=${
                        password[shareObj.index]
                    }`,
                    {
                ...
                .catch(async (error) => {
                    this.log.error(
                        ` send status for camshot could not be sent => ${error.message}, stack: ${error.stack}`,
                    );
            ...
            await this.setStateAsync(`device.${tabletName[shareObj.index]}.device_info.camshot64`, {
                val: htmlBase64,
                ack: true,
            });
            // @ts-ignore
            const base64String = Buffer.from(base64, 'base64');

            this.writeFile(
                `fully-tablet-control.admin`,
                `media/camshot_${tabletName[shareObj.index]}_${imageNr[shareObj.index]}.png`,
                base64String,
                (err) => {
                    // sentry issue https://sentry.io/organizations/issi/issues/3397930131/events/9c1490501d9c43a4acab7444e6ae23cb/?project=5217020
                    // if (err) this.log.error(err);
                    if (err) this.writeLog(`[writeFile] : ${err.message}, stack: ${err.stack}`, 'error');
                    ...
                    switch (shareObj.mode) {
                        case true:
                            ...
                            if (
                                imageNr[shareObj.index] === shareObj.series_shot_safe ||
                                imageNr[shareObj.index] === 30
                            ) {
                        ...
                        case false:
                            ...
                            if (
                                imageNr[shareObj.index] === shareObj.single_shot_safe ||
                                imageNr[shareObj.index] === 30
                            ) {
                                imageNr[shareObj.index] = 1;
                            } else {
            ...
            imageObj[`Nr${imageNr[shareObj.index]}`] = `/fully-tablet-control.admin/media/camshot_${
                tabletName[shareObj.index]
            }_${imageNr[shareObj.index]}.png`;
            this.log.debug(
                `File created in: /fully-tablet-control.admin.media/camshot_${tabletName[shareObj.index]}_${
                    imageNr[shareObj.index]
                }.png`,
            );
            await this.setState(`device.${tabletName[shareObj.index]}.device_info.camshotUrl`, {
                val: JSON.stringify(imageObj),
                ack: true,
            });
        } catch (error) {
            this.log.error(`[image_capture] : ${error.message}, stack: ${error.stack}`);
        }
    }
    ...
    async foregroundApp(index, value) {
        try {
            ...
            foregroundAppTimer[index] = setTimeout(async () => {
                this.log.debug(
                    `Axios request is executed for ${tabletName[index]} with the URL => ${foregroundAppUrl}`,
                );
                await axios
                    .get(foregroundAppUrl)
                    .then(async (result) => {
                        this.log.debug(
                            `${tabletName[index]} send status for toForeground = status Code: ${result.status} => status Message: ${result.statusText}`,
                        );
                        console.log(
                            `${tabletName[index]} send status for toForeground = status Code: ${result.status} => status Message: ${result.statusText}`,
                        );
                    ...
                    .catch(async (error) => {
                        this.log.error(
                            `${tabletName[index]} send status for toForeground could not be sent => ${error.message}, stack: ${error.stack}`,
                        );
                        console.log(
                            `${tabletName[index]} send status for reloadAll could not be sent [ val: ${value} ] => ${error.message}, stack: ${error.stack}`,
                        );
                    });
            }, fireTabletInterval);
        } catch (error) {
            this.log.error(
                `foregroundApp for ${tabletName[index]} has a problem: ${error.message}, stack: ${error.stack}`,
            );
        }
    }
        ...
        try {
            if (deviceInfo.length !== 0) {
                ...
                if (iobrokerLatitude !== '' && iobrokerLongitude !== '') {
                    this.log.debug(
                        `The latitude and longitude were read from the config. => latitude: ${iobrokerLatitude} | longitude: ${iobrokerLongitude}`,
                    );
                    ...
                    this.log.warn(
                        `The coordinates are not stored in the admin. Astro time is not switched on. Manuel mode is now active`,
                    );
                    timeMode = false;
                }
                ...
                if (timeMode) {
                    this.log.debug(
                        `The current time is now checked whether it is in the time range from ${dayH}:${dayM}:${dayS} to ${nightH}:${nightM}:${nightS}`,
                    );
                    const astro_Time = await this.time_range(
                        `${dayH}:${dayM}:${dayS}`,
                        ``,
                        `${nightH}:${nightM}:${nightS}`,
                    );
                    ...
                    this.log.debug(
                        `The current time is now checked whether it is in the time range from ${dayTime}:00:00 to ${midTime}:00:00 and ${nightTime}:00:00`,
                    );
                    const manuel_Time = await this.time_range(
                        `${dayTime}:00:00`,
                        `${midTime}:00:00`,
                        `${nightTime}:00:00`,
                    );
            ...
            switch (typ) {
                case 'boolean': {
                    ...
                    if (day_Time === 1 || day_Time === 2) {
                        if (controlMode) {
                            ...
                            // @ts-ignore
                            await this.manuel_Bri(index, stateValue);
                        } else {
                            this.log.debug(`night and day timeout are reset`);
                            if (automatic_briTimeout) clearTimeout(automatic_briTimeout);
                ...
                case 'number': {
                    if (controlMode) {
                        ...
                        this.log.debug(`start manuel brightness function`);
                        // @ts-ignore
                        await this.manuel_Bri(index, value);
     ...
     * @param {number} value
     */
    async manuel_Bri(index, value) {
        try {
            this.log.debug(
                `Manual brightness for ${tabletName[index]} is now carried out. Initialization operated .....`,
            );
            ...
            if (chargeDeviceValue[index]) {
                ...
                let chargingBri = Math.round(
                    await this.convert_brightness(value - brightnessN[index]['loadingLowering']),
                );
                this.log.debug(
                    `Charging brightness for ${
                        tabletName[index]
                    } has been calculated => ${chargingBri} and in percent => ${
                        value - brightnessN[index]['loadingLowering']
                    } `,
                );
                if (chargingBri <= 0) {
                    this.log.debug(
                        `The brightness for ${tabletName[index]} is less than 0 => ${chargingBri} is now replaced with 0`,
                    );
                    chargingBri = 0;
                ...
                if (screenSaverBriSync && screenSaverON) {
                    ...
                    if (chargingBri !== brightness[index]) {
                        ...
                        await axios
                            .get(ScreensaverOnBri)
                            .then(async (result) => {
                                this.log.debug(
                                    `${tabletName[index]} send status for ScreensaverOnBri = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for ScreensaverOnBri = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for ScreensaverOnBri name could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for ScreensaverOnBri name could not be sent val: ${value} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });
                        ...
                        await axios
                            ...
                            .then(async (result) => {
                                this.log.debug(
                                    `${tabletName[index]} send status for BrightnessURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for BrightnessURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                );
                            ...
                            .catch(async (error) => {
                                this.log.error(
                                    `${tabletName[index]} send status for BrightnessURL could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                                console.log(
                                    `${tabletName[index]} send status for BrightnessURL could not be sent val: ${value} ] => ${error.message}, stack: ${error.stack}`,
                                );
                            });

                        this.log.debug(
                            `Start the stateRequest in 300 ms to get the current values of the ${tabletName[index]} device`,
                        );
                ...
                const brightnessValue = Math.round(await this.convert_brightness(value));
                this.log.debug(
                    `The brightness for ${tabletName[index]} has now been calculated => ${brightnessValue} and in percent => ${value}`,
                );
            ...
            if (brightnessControlEnabled) {
                ...
                if (day_Time === 1) {
                    try {
                        ...
                        // @ts-ignore
                        if (
                            (!brightnessD && brightnessD.length !== 0) ||
                            (brightnessD !== [] && brightnessD.length !== 0)
                        ) {
                            for (const d in deviceEnabled) {
                                if (deviceEnabled[d] && !logMessage[d]) {
                                    if (enabledBrightness[d]) {
                                        this.log.debug(
                                            `automatic brightness control for ${tabletName[d]} is switched on the manual control is now switched off`,
                                        );
                                        await this.setStateAsync(
                                            `device.${tabletName[d]}.brightness_control_mode`,
                                            false,
                                            true,
                                        );
                                        ...
                                        if (brightnessD[d]) {
                                            ...
                                            if (chargeDeviceValue[d]) {
                                                newBrightnessD = Math.round(
                                                    await this.convert_brightness(
                                                        brightnessD[d]['dayBrightness'] -
                                                            brightnessD[d]['loadingLowering'],
                                                    ),
                                                );
                                                if (newBrightnessD <= 0) {
                                                    newBrightnessD = 0;
                                                    this.log.debug(
                                                        `brightness from ${tabletName[d]} is less than 0 brightness is set to`,
                                                    );
                                                }
                                            } else {
                                                newBrightnessD = Math.round(
                                                    await this.convert_brightness(brightnessD[d]['dayBrightness']),
                                                );
                                                this.log.debug(
                                                    `${tabletName[d]} brightness set on: ${newBrightnessD}[${brightnessD[d]['dayBrightness']}%]`,
                                                );
                                            }
                                            ...
                                            this.log.debug(
                                                `Check whether the screen saver and the brightness synchronization is switched on`,
                                            );
                                            if (enableScreenSaverBrightness[d] && screenSaverON) {
                                                this.log.debug(
                                                    `Check whether the current brightness is the same as the one you want to adjust`,
                                                );
                                                if (newBrightnessD !== brightness[d]) {
                                                    this.log.debug(
                                                        `send the screen saver brightness to the device =>${tabletName[d]}`,
                                                    );
                                                    await axios
                                                        .get(ScreensaverOnBri)
                                                        .then(async (result) => {
                                                            this.log.debug(
                                                                `${tabletName[d]} send status for ScreensaverOnBri = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                            );
                                                            console.log(
                                                                `${tabletName[d]} send status for ScreensaverOnBri = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                            );
                                                        ...
                                                        .catch(async (error) => {
                                                            this.log.error(
                                                                `${tabletName[d]} send status for ScreensaverOnBri name could not be sent => ${error.message}, stack: ${error.stack}`,
                                                            );
                                                            console.log(
                                                                `${tabletName[d]} send status for ScreensaverOnBri name could not be sent => ${error.message}, stack: ${error.stack}`,
                                                            );
                                                    ...
                                                    this.log.debug(
                                                        `send the screen brightness to the device =>${tabletName[d]}`,
                                                    );
                                                    await axios
                                                        .get(BrightnessURL)
                                                        .then(async (result) => {
                                                            this.log.debug(
                                                                `${tabletName[d]} send status for BrightnessURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                            );
                                                            console.log(
                                                                `${tabletName[d]} send status for BrightnessURL = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                            );
                                                        ...
                                                        .catch(async (error) => {
                                                            this.log.error(
                                                                `${tabletName[d]} send status for BrightnessURL could not be sent => ${error.message}, stack: ${error.stack}`,
                                                            );
                                                            console.log(
                                                                `${tabletName[d]} send status for BrightnessURL could not be sent => ${error.message}, stack: ${error.stack}`,
                                                            );
                                                    ...
                                                    this.log.debug(
                                                        `Start the stateRequest in 300 ms to get the current values of the ${tabletName[d]} device`,
                                                    );
                                                    if (BriRequestTimeout) clearTimeout(BriRequestTimeout);
                                                    BriRequestTimeout = setTimeout(async () => {
                                                        await this.stateRequest();
                                        ...
                                        this.log.debug(
                                            `Automatic brightness has been deactivated for ${tabletName[d]} and is now switched to manual control`,
                                        );
                                        await this.setStateAsync(
                                            `device.${tabletName[d]}.brightness_control_mode`,
                                            true,
                            ...
                            for (const d in deviceEnabled) {
                                if (!enabledBrightness[d]) {
                                    this.log.warn(
                                        `Automatic brightness for ${tabletName[d]} is activated but no configuration is entered, the manual mode is activated.`,
                                    );
                                    await this.setStateAsync(
                                        `device.${tabletName[d]}.brightness_control_mode`,
                                        true,
                                ...
                                if (deviceEnabled[d] && !logMessage[d]) {
                                    if (enabledBrightness[d]) {
                                        ...
                                        if (brightnessD[d]) {
                                            ...
                                            if (chargeDeviceValue[d]) {
                                                newBrightnessM = Math.round(
                                                    await this.convert_brightness(
                                                        brightnessD[d]['midTimeBrightness'] -
                                                            brightnessD[d]['loadingLowering'],
                                                    ),
                                                );
                                                if (newBrightnessM <= 0) {
                                                ...
                                                newBrightnessM = Math.round(
                                                    await this.convert_brightness(brightnessD[d]['midTimeBrightness']),
                                                );
                                                this.log.debug(
                                                    `${tabletName[d]} brightness set on: ${newBrightnessM}[${brightnessD[d]['midTimeBrightness']}%]`,
                                                );
                                            }
                    ...
                    try {
                        ...
                        // @ts-ignore
                        if (
                            (!brightnessN && brightnessN.length !== 0) ||
                            (brightnessN !== [] && brightnessN.length !== 0)
                        ) {
                            for (const d in deviceEnabled) {
                                if (deviceEnabled[d] && !logMessage[d]) {
                                    if (enabledBrightness[d]) {
                                        ...
                                        if (brightnessN[d]) {
                                            ...
                                            if (chargeDeviceValue[d]) {
                                                newBrightnessN = Math.round(
                                                    await this.convert_brightness(
                                                        brightnessN[d]['nightBrightness'] -
                                                            brightnessN[d]['loadingLowering'],
                                                    ),
                                                );
                                                if (newBrightnessN <= 0) {
                                                    newBrightnessN = 0;
                                                ...
                                                newBrightnessN = Math.round(
                                                    await this.convert_brightness(brightnessN[d]['nightBrightness']),
                                                );
                                                this.log.debug(
                                                    `${tabletName[d]} brightness set on: ${newBrightnessN}[${brightnessN[d]['nightBrightness']}%]`,
                                                );
                                            }
                ...
                for (const d in deviceEnabled) {
                    this.log.debug(`automatic_bri function is stopped switch to manual mode`);
                    await this.setStateAsync(`device.${tabletName[d]}.brightness_control_mode`, true, true);
                    const manualBri = await this.getStateAsync(`device.${tabletName[d]}.manualBrightness`);
                    if (manualBri) {
                        const manualBriVal = manualBri.val;
                        await this.manuel_Bri(d, Number(manualBriVal));
                    }
                }
            }
        }
    ...
    async screenOn(index) {
        try {
            ...
            if (screen_on) {
                this.log.warn(
                    `Attention the screen on ${tabletName[index]} has been switched off, an attempt is made to switch it on again`,
                );
                await axios
                    .get(Screen)
                    .then(async (result) => {
                        this.log.debug(
                            `${tabletName[index]} send status for screenOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                        );
                        console.log(
                            `${tabletName[index]} send status for screenOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                        );
                    ...
                    .catch(async (error) => {
                        this.log.error(
                            `${tabletName[index]} send status for screenOn could not be sent => ${error.message}, stack: ${error.stack}`,
                        );
                        console.log(
                            `${tabletName[index]} send status for screenOn could not be sent => ${error.message}, stack: ${error.stack}`,
                        );
            ...
            if (motionSensor_enabled) {
                ...
                if ((!motion && motion.length !== 0) || (motion !== [] && motion.length !== 0)) {
                    ...
                    if (motion[index]['enabled']) {
                        ...
                        this.log.debug(
                            `the motion detector with the id: ${motion[index]['motionid']} is deactivated !!`,
                        );
                    }
                    ...
                    for (const on in ip) {
                        ...
                        if (deviceEnabled[on] && !logMessage[on]) {
                            ...
                            if (motionSensor_enabled) {
                                ...
                                if (!motionVal[on]) {
                                    this.log.debug(
                                        `Check whether the screen saver is switched on on the ${tabletName[on]}`,
                                    );
                                    if (!isInScreensaver[on]) {
                                        this.log.debug(`build url for ${tabletName[on]}`);

                                        this.log.debug(
                                            `${tabletName[on]} Screensaver starts in ${screenSaverTime[on]} ms ==> ${
                                                screenSaverTime[on] / 60000
                                            }`,
                                        );
                                        ...
                                        screensaverTimer[on] = setTimeout(async () => {
                                            ...
                                            await axios
                                                .get(screensaverOnURL[on])
                                                .then(async (result) => {
                                                    this.log.debug(
                                                        `${tabletName[on]} send status for ScreensaverOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                    );
                                                    console.log(
                                                        `${tabletName[on]} send status for ScreensaverOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                    );
                                                ...
                                                .catch(async (error) => {
                                                    if (!logMessage[on])
                                                        this.log.error(
                                                            `${tabletName[on]} send status for ScreensaverOn could not be sent => ${error.message}, stack: ${error.stack}`,
                                                        );
                                                });

                                            this.log.debug(
                                                `Start the stateRequest in 300 ms to get the current values of the ${tabletName[on]} device`,
                                            );
                                    ...
                                    this.log.debug(
                                        `Movement was detected the screen saver is switched off for ${tabletName[on]}`,
                                    );

                                    this.log.debug(`build url for ${tabletName[on]}`);

                                    this.log.debug(
                                        `Check whether the ${tabletName[on]} screen saver is already switched off`,
                                    );
                                    if (!isInScreensaver[on]) {
                                        this.log.debug(`${tabletName[on]} screensaver is already off`);
                                    } else if (isInScreensaver[on]) {
                                        await axios
                                            .get(screensaverOffURL[on])
                                            .then(async (result) => {
                                                this.log.debug(
                                                    `${tabletName[on]} send status for ScreensaverOff = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                );
                                                console.log(
                                                    `${tabletName[on]} send status for ScreensaverOff = status Code: ${result.status} => status Message: ${result.statusText}`,
                                                );
                                            ...
                                            .catch(async (error) => {
                                                if (!logMessage[on])
                                                    this.log.error(
                                                        `${tabletName[on]} send status for ScreensaverOff could not be sent => ${error.message}, stack: ${error.stack}`,
                                                    );
                                            });
                                    }
                                }
    ...
    async screensaverManuel(index) {
        ...
        if (screenSaverOn) {
            ...
            this.log.debug(
                `${tabletName[index]} Screensaver starts in ${screenSaverTime[index]} ms ==> ${
                    screenSaverTime[index] / 60000
                }`,
            );
            ...
            screensaverTimer[index] = setTimeout(async () => {
                ...
                if (!isInScreensaver[index]) {
                    await axios
                        .get(screensaverOnURL[index])
                        .then(async (result) => {
                            this.log.debug(
                                `${tabletName[index]} send status for ScreensaverOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                            console.log(
                                `${tabletName[index]} send status for ScreensaverOn = status Code: ${result.status} => status Message: ${result.statusText}`,
                            );
                        ...
                        .catch(async (error) => {
                            if (!logMessage[index])
                                this.log.error(
                                    `${tabletName[index]} send status for ScreensaverOn could not be sent => ${error.message}, stack: ${error.stack}`,
                                );
                        });
                }
                                ...
                                if (chargerid) {
                                    ...
                                    if (bat <= loadStart && !chargeDeviceValue[index]) {
                                        switch (typeof chargeDevice.val) {
                                            case 'number':
                                                this.log.debug(
                                                    `Battery is at the start of charging limit start charging`,
                                                );
                                        ...
                                        switch (typeof chargeDevice.val) {
                                            case 'number':
                                                this.log.debug(
                                                    `Battery level has reached the set charging stop, stop charging`,
                                                );
                                ...
                                if (chargerid) {
                                    switch (typeof chargeDevice.val) {
                                        case 'number':
                                            messageCharging[index] = false;
                                            if (!chargeDeviceValue[index])
                                                this.log.debug(`The adapter now switches on the socket`);
                                            if (!chargeDeviceValue[index])
                                                await this.setForeignStateAsync(chargerid, 1, false);
                                            if (!chargeDeviceValue[index])
                                                this.log.debug(`${tabletName[index]} Continuous current`);
                                            break;
                                    ...
                                    this.log.warn(
                                        `${tabletName[index]} Charger ID for Continuous current not specified`,
                                    );
                                }
                        ...
                        if (power_mode !== 'off') {
                            if (chargeDevice !== null && chargeDevice !== undefined) {
                                if (telegram_enabled === true) {
                                    if (
                                        (bat <= 18 && !chargeDeviceValue[index] && !telegramStatus[index]) ||
                                        (bat <= 18 && chargeDeviceValue[index] && !telegramStatus[index])
                                    ) {
                                        switch (typeof chargeDevice.val) {
                                            case 'number':
                                                ...
                                                this.sendTo('telegram.0', 'send', {
                                                    text:
                                                        this.formatDate(new Date(), 'TT.MM.JJ SS:mm') +
                                                        ` ${tabletName[index]} Tablet charging function has detected a malfunction, the tablet is not charging, please check it !!!`,
                                                    user: User,
                                                });
                                                this.log.warn(
                                                    this.formatDate(new Date(), 'TT.MM.JJ SS:mm') +
                                                ...
                                                await this.setForeignStateAsync(chargerid, 1, false);
                                                this.setState(`device.${tabletName[index]}.charging_warning`, {
                                                    val: true,
                                                    ack: true,
                                                });
                                        ...
                                        this.sendTo('telegram.0', 'send', {
                                            text:
                                                this.formatDate(new Date(), 'TT.MM.JJ SS:mm') +
                                                ` ${tabletName[index]} Tablet is charging the problem has been fixed.`,
                                            user: User,
                                        });
                                        this.log.warn(
                                            this.formatDate(new Date(), 'TT.MM.JJ SS:mm') +
                                    ...
                                    if (
                                        (bat <= 18 && !chargeDeviceValue[index] && !AlertMessageSend[index]) ||
                                        (bat <= 18 && chargeDeviceValue[index] && !AlertMessageSend[index])
                                    ) {
                                        switch (typeof chargeDevice.val) {
                                            case 'number':
    ...
    async time_range(startTime, midTime, endTime) {
        ...
        // Format the end date
        const midDate = new Date(currentDate.getTime());
        // @ts-ignore
    ...
    /**
     * Convert Persentage to Brightness
     * @param brightness
     * @returns {Promise<number>}
     */
    async convert_brightness(brightness) {
        try {
            const brightness_value = Math.round((brightness / 100) * 255);
            if (Number.isNaN(brightness_value)) {
                this.log.error(`[convert_brightness] : brightness is NaN => ${brightness} set to 0`);
                return 0;
            }
            return brightness_value;
        } catch (error) {
            this.log.error(`convert_brightness has a problem: ${error.message}, stack: ${error.stack} `);
            return 0;
        }
    }
    ...
    async replaceFunction(str) {
        let replaceStr = str.toLowerCase();
        replaceStr = replaceStr.replace(/ä/g, 'ae');
        replaceStr = replaceStr.replace(/ö/g, 'oe');
        replaceStr = replaceStr.replace(/ü/g, 'ue');
        replaceStr = replaceStr.replace(/ß/g, 'ss');
        replaceStr = replaceStr.replace(/[^a-z0-9_]/g, '_');
        return replaceStr;
    }
            ...
            if (state) {
                ...
                for (const index in tabletName) {
                    if (
                        ...
                        if (
                            id === `${this.namespace}.device.${tabletName[index]}.manualBrightness` ||
                            id === `${this.namespace}.device.${tabletName[index]}.brightness_control_mode`
                        ) {
    ...
    /**
     * @description call the wirteLog function to write the log message to info / debug / warn / error log
     * @param log message
     * @param type - info / debug / warn / error
     */
    writeLog(log, type) {
        if (type === 'error') {
            this.log.error(log);
                ...
                case 'pingTablet': {
                    ...
                    for (const i in deviceObj) {
                        if (deviceObj[i].enabled) {
                            await axios
                                .get(
                                    `http://${deviceObj[i].ip}:${deviceObj[i].port}/?cmd=deviceInfo&type=json&password=${deviceObj[i].password}`,
                                )
                                .then(async (apiResult) => {
